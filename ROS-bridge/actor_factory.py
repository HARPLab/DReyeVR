...
from carla_ros_bridge.sensor import Sensor  # for custom sensors
...
class ActorFactory(object):
    ...
    def update(self):
        ...
        previous_actors = set(
            [x.uid for x in self.actors.values() if isinstance(x, Actor)])
    ...
    def create(self, type_id, name, attach_to, spawn_pose, carla_actor=None):
        with self.lock:
            ...
            actor = self._create_object(
                uid, type_id, name, parent, spawn_pose, carla_actor)
            self.actors[actor.uid] = actor

            rospy.loginfo("Created {}(id={})".format(
    ...
    def destroy(self, actor_id):
        with self.lock:
            ...
            rospy.loginfo("Removed {}(id={})".format(
                actor.__class__.__name__, actor.uid))
