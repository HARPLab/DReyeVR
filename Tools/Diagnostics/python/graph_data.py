...
def plot_versus(data_x, data_y, units="", name_x="X", name_y="Y", trim=(0, 0), lines=False, colour="r", dir_path="results"):
...
def plot_diff(subA, subB, units="", name_A="A", name_B="B", trim=(0, 0), lines=False, colour="r", dir_path="results"):
    # trim the starts and end of data
    trim_start, trim_end = trim
    ...
    plt.yticks()
    plt.tick_params(labelsize=15)
    plt.title("Difference (" + name_A + " - " + name_B + ")" + trim_str,
              fontsize=18)
...
def plot_alone(data, units="", name="Y", trim=(0, 0), lines=False, colour="r", dir_path="results"):
    x_values = np.arange(len(data))
    plot_versus(x_values, data, units=units, name_x="",
                name_y=name, trim=trim, lines=lines, colour=colour,
                dir_path=dir_path)
...
def sr_no_carla(folder="without_carla", delimiter=" "):
    ...
    ue4 = read_from_file(datadir, "ue4",
                         delimiter=delimiter)
    sranipal = read_from_file(datadir, "sranipal",
                              delimiter=delimiter)
    ...
    plot_versus(ue4, sranipal, units="s", name_x="ue4",
                name_y="sranipal", dir_path=output_path)
    # plot ue4 - sranipal on the y
    plot_diff(ue4, sranipal, units="s", name_A="ue4",
              name_B="sranipal", trim=(5, 0), dir_path=output_path)
    # plot framseq on the y
    plot_alone(frameseq, name="frameseq", dir_path=output_path)
    # plot ue4 (x) vs frameseq (y)
    plot_versus(ue4, frameseq, name_x="ue4", name_y="frameseq",
                trim=(5, 0), dir_path=output_path)
...
def sr_with_carla(data_folders=[], output_folder="with_carla", delimiter=", "):
    ...
    for folder in data_folders:
        ...
        carla = read_from_file(datadir, "carla",
                               delimiter=delimiter)
        sranipal = read_from_file(datadir, "sranipal",
                                  delimiter=delimiter)
        carla_stream = read_from_file(datadir, "carla_stream",
                                      delimiter=delimiter)
        frameseq = read_from_file(datadir, "frame_seq",
                                  delimiter=delimiter)
        # ensure all lengths are the same
        max_len = min(len(carla), len(sranipal),
                      len(carla_stream), len(frameseq))
        carla = carla[:max_len]
        ...
        plot_versus(carla, sranipal, units="s",
                    name_x="carla", name_y="sranipal", dir_path=specific_output_path)
        # plot carla - sranipal on the y
        plot_diff(carla, sranipal, units="s", name_A="carla",
                  name_B="sranipal", trim=(5, 0), dir_path=specific_output_path)
        # plot carla stream on the y
        plot_alone(carla_stream, name="carla_stream",
                   dir_path=specific_output_path)
        # plot carla (x) vs carla_stream (y)
        plot_versus(carla, carla_stream, name_x="carla",
                    name_y="carla_stream", trim=(5, 0), dir_path=specific_output_path)
        # plot framseq on the y
        plot_alone(frameseq, name="frame_seq", dir_path=specific_output_path)
        # plot ue4 (x) vs frameseq (y)
        plot_versus(carla, frameseq, name_x="carla", name_y="frame_seq",
                    trim=(5, 0), dir_path=specific_output_path)
        # plot framerate of frameseq over time
        frameseq_fps = np.diff(frameseq) / np.diff(carla)
        plot_alone(frameseq_fps, name="frameseq_fps",
                   dir_path=specific_output_path)
        """Derivatives"""
        # plot differences between sranipal points
        framerates(sranipal, name="sranipal",
                   trim=(0, 10), dir_path=specific_output_path)
        # plot differences between carla points
        framerates(carla, name="carla", trim=(10, 10),
                   dir_path=specific_output_path)
        # plot differences between carla_stream points
        framerates(carla_stream, name="carla_stream", trim=(10, 10),
                   dir_path=specific_output_path)
        # plot differences between frameseq points
        framerates(frameseq, name="frame_seq",
                   trim=(10, 10), dir_path=specific_output_path)
        """Metadata"""
        # plot framerates wrt carla_stream (time between tick and stream)
        plot_alone(1 / np.diff(carla_stream), name="fps_carla_stream",
                   dir_path=specific_output_path)
        # plot framerates wrt sranipal time (time between tick and stream)
        plot_alone(1 / np.diff(sranipal), name="fps_sranipal",
                   dir_path=specific_output_path)
        # plot framerates wrt carla time (time between tick and stream)
        plot_alone(1 / np.diff(carla), name="fps_carla",
                   dir_path=specific_output_path)
...
if __name__ == '__main__':
    # print("SRanipal no Carla")
    # sr_no_carla()

    # print("SRanipal + Carla test 1")
    # sr_with_carla(data_folders=["timing_data_6.3ms_fast",
    #                             "timing_data_10.5ms_med",
    #                             "timing_data_34.5ms_slow"],
    #               delimiter=", ")

    print("SRanipal + Carla test 2")
    sr_with_carla(data_folders=["timing_data_fast",
                                "timing_data_med",
                                "timing_data_slow"],
                  delimiter=", ")
